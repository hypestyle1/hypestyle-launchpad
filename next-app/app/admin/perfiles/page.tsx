    ...
    <div className="">
      <div className="bg-card border-b border-border px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-2 sticky top-0 z-10">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <span className="text-[14px] font-semibold text-foreground">Perfiles</span>
      ...
      <div className="max-w-[900px] mx-auto px-4 py-8">
        <p className="text-[12px] text-muted-foreground mb-6 leading-relaxed max-w-[560px]">
          Cada perfil entra con su propio mail y contraseña desde{' '}
          <Link href="/admin/login" className="underline hover:text-foreground">/admin/login</Link>. El tipo de acceso decide
          qué secciones ve.
        </p>
        ...
        {mostrarAlta && (
          <form onSubmit={crearPerfil} className="bg-card rounded-lg border border-border p-5 mb-6 space-y-3">
            <p className="text-[13px] font-semibold text-foreground">Nuevo perfil</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <input required placeholder="Nombre" value={nuevo.name} onChange={e => setNuevo(n => ({ ...n, name: e.target.value }))} className="border border-border-mid rounded-md px-3 py-2 text-[13px] focus:outline-none focus:border-ring" />
              <input required type="email" placeholder="Mail" value={nuevo.email} onChange={e => setNuevo(n => ({ ...n, email: e.target.value }))} className="border border-border-mid rounded-md px-3 py-2 text-[13px] focus:outline-none focus:border-ring" />
            </div>
            <input required type="text" placeholder="Contraseña inicial (mínimo 8)" value={nuevo.password} onChange={e => setNuevo(n => ({ ...n, password: e.target.value }))} className="w-full border border-border-mid rounded-md px-3 py-2 text-[13px] font-mono focus:outline-none focus:border-ring" />
            <div className="flex flex-wrap gap-2">
              {ROLES.map(r => (
                <button
                  ...
                  className={`text-[11px] px-3 py-1.5 rounded-full border ${nuevo.role === r.value ? 'bg-primary text-primary-foreground border-foreground' : 'border-border-mid text-muted-foreground hover:border-border0'}`}
                >
                  {r.label}
                </button>
              ))}
            </div>
            <p className="text-[11px] text-muted-foreground/70">{ROLES.find(r => r.value === nuevo.role)?.detalle}</p>
            {error && <p className="text-[12px] text-red-600">{error}</p>}
            <button type="submit" disabled={guardandoId === 'nuevo'} className="bg-primary text-primary-foreground rounded-md px-4 py-2 text-[12px] font-semibold hover:opacity-90 disabled:opacity-50">
        ...
        <div className="space-y-3">
            ...
            <div key={p.id} className="bg-card rounded-lg border border-border p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-[180px]">
                  <p className="text-[14px] font-semibold text-foreground">{p.name || p.email}</p>
                  ...
                  <p className="text-[11px] text-muted-foreground/70 mt-1">
                    {p.loginCount > 0
                      ...
                      : 'nunca entró'}
                    {p.lastLogin && <span className="text-muted-foreground/50"> · {fmtFecha(p.lastLogin)}</span>}
                  </p>
                </div>
                  ...
                  {ROLES.map(r => (
                    <button
                      ...
                      className={`text-[11px] px-3 py-1.5 rounded-full border transition-colors disabled:opacity-40 ${
                        p.role === r.value ? 'bg-primary text-primary-foreground border-foreground' : 'border-border-mid text-muted-foreground hover:border-border0'
                      }`}
                    >
                      {r.label}
                    </button>
          ...
          {!cargando && perfiles.length === 0 && (
            <p className="text-[13px] text-muted-foreground/70 text-center py-12">
