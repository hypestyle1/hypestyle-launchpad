  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="bg-card rounded-lg shadow-sm border border-border p-8 w-full max-w-sm">
        ...
        <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground/70 text-center mb-7">
        ...
        {modo === 'olvide' ? (
          enviado ? (
            <div className="text-center">
              <p className="text-[14px] font-semibold text-foreground mb-2">Revisá tu casilla</p>
              ...
              <button
                ...
                className="text-[12px] text-muted-foreground/70 hover:text-foreground mt-6 underline"
              >
                Volver al ingreso
              </button>
            </div>
            ...
            <form onSubmit={pedirLink} className="space-y-2.5">
              <p className="text-[12px] text-muted-foreground leading-relaxed pb-1">
              ...
              <input
                ...
                autoFocus
                required
                className="w-full border border-border-mid rounded-md px-3 py-2.5 text-[13px] focus:outline-none focus:border-ring"
              />
              {error && <p className="text-[12px] text-red-600 pt-1">{error}</p>}
              <button type="submit" disabled={loading} className="w-full bg-primary text-primary-foreground rounded-md py-2.5 text-[12px] font-bold uppercase tracking-[0.1em] hover:opacity-90 disabled:opacity-50 !mt-5">
                {loading ? '...' : 'Enviar link'}
              </button>
              <div className="text-center !mt-4">
                <button type="button" onClick={() => { setModo('login'); setError(''); }} className="text-[12px] text-muted-foreground/70 hover:text-foreground underline">
                  Volver al ingreso
                </button>
              </div>
            </form>
        ...
        <form onSubmit={handleSubmit} className="space-y-2.5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-primary text-primary-foreground rounded-md py-2.5 text-[12px] font-bold uppercase tracking-[0.1em] hover:opacity-90 disabled:opacity-50 !mt-5"
          >
            {loading ? '...' : 'Entrar'}
          </button>
          ...
          <div className="text-center !mt-4">
            <button
              ...
              className="text-[12px] text-muted-foreground/70 hover:text-foreground underline"
            >
