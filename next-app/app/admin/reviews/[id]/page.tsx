...
const MODERATION_COLORS: Record<string, string> = {
  ...
  spam: 'bg-red-100 text-red-700', trash: 'bg-muted text-muted-foreground',
};
...
export default function ReviewDetailPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-[13px] text-muted-foreground/70">Cargando...</div>;
    ...
    <div className="min-h-screen bg-background">
      <div className="bg-card border-b border-border px-6 py-3 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[14px] font-semibold text-foreground">Solicitud #{detail.id} — Orden #{detail.order.number}</span>
        </div>
        <Link href={`/admin/pedidos/${detail.order.id}`} className="text-[12px] text-muted-foreground/70 hover:text-foreground">Ver pedido →</Link>
      </div>
      ...
      <div className="max-w-[900px] mx-auto px-4 py-5 space-y-4">
        ...
        <div className="bg-card rounded-lg border border-border px-5 py-4">
          <h2 className="text-[13px] font-semibold text-foreground mb-3">Datos de la solicitud</h2>
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[12px]">
            <div><span className="text-muted-foreground/70">Cliente:</span> {detail.customer_name || '—'}</div>
            <div><span className="text-muted-foreground/70">Email:</span> {detail.customer_email}</div>
            <div><span className="text-muted-foreground/70">Estado:</span> <span className="font-medium">{detail.status_label}</span></div>
            <div><span className="text-muted-foreground/70">Fecha de la orden:</span> {fmtDate(detail.order.date)}</div>
            <div><span className="text-muted-foreground/70">Despacho:</span> {fmtDate(detail.dispatched_at)}</div>
            <div><span className="text-muted-foreground/70">Origen del despacho:</span> {detail.dispatched_source ? (DISPATCH_SOURCE_LABELS[detail.dispatched_source] || detail.dispatched_source) : '—'}</div>
            <div><span className="text-muted-foreground/70">Programada para:</span> {fmtDate(detail.scheduled_for)}</div>
            <div><span className="text-muted-foreground/70">Enviada:</span> {fmtDate(detail.sent_at)}</div>
            <div><span className="text-muted-foreground/70">Apertura:</span> {detail.opened_at ? fmtDate(detail.opened_at) : 'Sin medición confiable disponible'}</div>
            <div><span className="text-muted-foreground/70">Respondida:</span> {fmtDate(detail.responded_at)}</div>
            <div><span className="text-muted-foreground/70">Link vence:</span> {fmtDate(detail.expires_at)}</div>
            {detail.fail_reason && <div className="col-span-2 text-red-600"><span className="text-muted-foreground/70">Motivo de falla:</span> {detail.fail_reason}</div>}
          </div>

          <div className="flex gap-2 flex-wrap mt-4">
            {(detail.status === 'sent' || detail.status === 'failed') && (
              <button onClick={() => doAction('resend')} disabled={busy} className="text-[12px] font-medium text-foreground/80 hover:text-foreground border border-border rounded-md px-3 py-1.5 disabled:opacity-40">
        ...
        <div className="bg-card rounded-lg border border-border px-5 py-4">
          <h2 className="text-[13px] font-semibold text-foreground mb-3">Productos reseñables ({detail.products.filter(p => p.reviewed).length}/{detail.products.length})</h2>
          <div className="space-y-3">
            {detail.products.map(p => (
              <div key={p.order_item_id} className="border border-border rounded-lg p-3">
        ...
        {/* Cupón */}
        <div className="bg-card rounded-lg border border-border px-5 py-4">
          <h2 className="text-[13px] font-semibold text-foreground mb-3">Cupón</h2>
          {detail.coupon ? (
            <div className="text-[12px] space-y-1">
              <div className="flex items-center gap-2">
                <span className="font-mono text-[14px] font-semibold">{detail.coupon.code}</span>
                <button onClick={copyCoupon} className="text-[11px] text-muted-foreground/70 hover:text-foreground border border-border rounded px-2 py-0.5">
                  {copied ? 'Copiado' : 'Copiar'}
                </button>
              </div>
              <div><span className="text-muted-foreground/70">Valor:</span> {detail.coupon.value}% OFF</div>
              <div><span className="text-muted-foreground/70">Vence:</span> {detail.coupon.expires_at || 'Sin vencimiento'}</div>
              <div><span className="text-muted-foreground/70">Uso:</span> {detail.coupon.usage_count}/{detail.coupon.usage_limit} {detail.coupon.used && '(ya usado)'}</div>
              <div><span className="text-muted-foreground/70">Uso individual:</span> {detail.coupon.individual_use ? 'Sí' : 'No'}</div>
            </div>
          ) : (
            <p className="text-[12px] text-muted-foreground/70">No se generó ningún cupón para esta solicitud.</p>
          )}
        </div>

        {/* Historial */}
        <div className="bg-card rounded-lg border border-border px-5 py-4">
          <h2 className="text-[13px] font-semibold text-foreground mb-3">Historial de eventos</h2>
          {detail.logs.length > 0 ? (
            <div className="text-[11px] font-mono text-muted-foreground space-y-1 max-h-64 overflow-y-auto">
              {detail.logs.map((line, i) => <div key={i}>{line}</div>)}
            </div>
          ) : (
            <p className="text-[12px] text-muted-foreground/70">Sin eventos registrados todavía.</p>
          )}
        </div>
      </div>
      ...
      {cancelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !busy && setCancelModal(false)}>
          <div className="bg-card rounded-lg p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-[13px] text-muted-foreground mb-5">No se puede deshacer. La solicitud queda marcada como cancelada, conservando su historial.</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setCancelModal(false)} disabled={busy} className="px-4 py-2 text-[13px] font-medium text-muted-foreground hover:bg-muted rounded-lg disabled:opacity-40">Volver</button>
