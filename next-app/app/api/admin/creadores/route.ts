...
import { listarCreadores, actualizarCreador, ESTADOS, GENEROS } from '@/lib/creadores';
...
export async function POST(req: NextRequest) {
  ...
  const { id, estado, nota, revisadoPor, genero } = await req.json().catch(() => ({}));
  ...
  // '' vuelve a "sin asignar"; undefined no lo toca.
  if (genero !== undefined && genero !== '' && !GENEROS.includes(genero)) {
    return NextResponse.json({ message: 'Género inválido' }, { status: 400 });
  }

  const res = await actualizarCreador(Number(id), { estado, nota, revisadoPor: revisadoPor || '', genero });
