...
import { PAYPAL_API, paypalAccessToken, getPayPalOrder } from '@/lib/paypal';
...
// Dashboard de PayPal, suscripto al evento PAYMENT.CAPTURE.COMPLETED). Ese
// registro nunca se hizo, así que HOY ESTA RUTA RECHAZA TODO. Mientras tanto el
// respaldo real es /api/paypal-reconcile, que consulta a PayPal por cron y no
// necesita ningún registro. Cuando se cargue PAYPAL_WEBHOOK_ID los dos conviven
// sin pisarse: fulfillOrder es idempotente.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (ppOrder.wcOrderId === null) {
      return NextResponse.json({ ok: false, error: 'no reference_id on paypal order' }, { status: 400 });
    }

    const result = await fulfillOrder(ppOrder.wcOrderId, 'paypal', paypalOrderId);
    return NextResponse.json({ ok: true, wcOrderId: ppOrder.wcOrderId, result: result.reason });
  } catch (err) {
