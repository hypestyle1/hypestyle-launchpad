// Decide si el 3x2 está activo: SOLO si Argentina ganó su último partido del Mundial,
// y se mantiene activo hasta que arranca el próximo (cuartos de final, fecha que el
// bracket todavía puede no haber publicado). No usa getArgentinaFixture() (corta a 24h
// los partidos terminados); usa la lista completa para sostener la ventana varios días.
...
export async function getPromo3x2Status(): Promise<Promo3x2Status> {
  ...
  if (!fixtures.length) return base('none');
