...
// retocar. Cada slide trae dos fotos: la horizontal para el card de desktop
// (990/503) y una vertical para mobile, donde el card ocupa 80dvh de alto y
// una horizontal quedaría recortada a una franja. pos = foco vertical del
// bg-cover; las horizontales son 3:2 y el card es casi 2:1, así que se pierde
// ~25% de alto y hay que elegir dónde.
const SLIDES = [
...
export default function HeroLookbookFW26() {
  ...
  // después del primer pintado. Se precarga la vertical: es la que pinta el SSR.
  preload(SLIDES[0].imgMobile, { as: 'image', fetchPriority: 'high' });
  ...
  // Desktop y mobile muestran fotos distintas del mismo slide (ver SLIDES).
  // Se resuelve en el cliente: en el SSR se pinta la vertical, que es la que
  // ve el 80% de las visitas (mobile), y desktop cambia apenas hidrata.
  useEffect(() => {
    ...
    <section ref={sectionRef} className="relative w-full h-[80dvh] md:h-[100dvh] -mt-[var(--offset)] overflow-hidden bg-bg-dark">
      ...
      <div className="absolute inset-0 flex items-center justify-center px-0 md:px-4">
        ...
        >
          {slides.map((s, i) => (
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: ready.has(i) ? `url('${desktop ? s.img : s.imgMobile}')` : undefined,
                  backgroundPosition: `center ${desktop ? s.pos : s.posMobile}`,
                }}
              />
            </div>
          ))}
          ...
          {/* Acceso al lookbook completo: la produ entera, en /lookbook-fw26. */}
          <Link
            href="/lookbook-fw26/"
            className="absolute right-4 bottom-4 md:right-8 md:bottom-8 z-20 hidden md:inline-flex items-center gap-2
                       text-white text-[12px] font-semibold uppercase tracking-[0.14em]
                       [text-shadow:0_1px_8px_rgba(0,0,0,0.6)] hover:opacity-80 transition-opacity"
          >
            Ver lookbook FW26
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M6 3l5 5-5 5" /></svg>
          </Link>
        </div>
      </div>
