...
export default function Promo3x2Section() {
  const { data, phase } = usePromo3x2Status();

  if (!phase || phase === 'lost' || phase === 'none') return null;
  ...
  return (
    <section
      ...
      style={{ background: `linear-gradient(160deg, #0d1b3d 0%, #1B3B6F 55%, ${CELESTE} 130%)` }}
    >
      {/* Noise */}
      <div
      ...
      <div
        ...
        style={{ background: `radial-gradient(ellipse at center, rgba(212,175,55,0.16) 0%, transparent 70%)` }}
      />
        ...
        {phase === 'pre' && (
          <>
            <div className="flex flex-col items-center gap-2">
              <p className="text-[13px] md:text-[16px] font-bold uppercase tracking-[0.14em] text-white/80">
                Si Argentina le gana a {match?.oppName || 'Egipto'} hoy
              </p>
              <h2
                className="font-black uppercase leading-none tracking-[-0.04em] text-white"
                style={{ fontSize: 'clamp(56px, 14vw, 160px)' }}
              >
                3<span style={{ color: GOLD }}>X</span>2
              </h2>
              <p className="font-black uppercase text-white/80" style={{ fontSize: 'clamp(16px, 3.5vw, 36px)', letterSpacing: '0.12em' }}>
                en todo el catálogo
              </p>
            </div>
        ...
        {phase === 'live' && match && (
          <>
            ...
            <p className="font-black uppercase text-white/70" style={{ fontSize: 'clamp(14px, 2.5vw, 24px)', letterSpacing: '0.1em' }}>
              Si Argentina gana, se activa el 3x2 en todo el catálogo
            </p>
          </>
        )}
        ...
        {phase === 'won' && (
          <>
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="font-black uppercase text-white/80" style={{ fontSize: 'clamp(16px, 3.5vw, 36px)', letterSpacing: '0.12em' }}>
                activo en todo el catálogo
              </p>
            </div>
            {data?.nextMatchDate ? (
              <Countdown targetISO={data.nextMatchDate} />
            ) : (
              <p className="text-[11px] tracking-[0.16em] uppercase text-white/50">
                Válido hasta el próximo partido de Argentina
              </p>
            )}
          </>
        )}
