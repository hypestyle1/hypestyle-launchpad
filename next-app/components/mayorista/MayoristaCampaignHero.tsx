...
import { BRAND_HERO, NEXT_DROP_LABEL } from '@/lib/mayorista-copy';
...
// retailers (badge, headline, "HASTA X% EXTRA", texto, fecha de cierre, CTA).
// Sin campaña: hero de marca con el teaser del próximo drop como línea
// secundaria. Estética del sitio: blanco, negro, bordes finos, sin colores de
// oferta.
    ...
    return (
      <section className="rounded-[16px] border border-border bg-bg-alt/40 px-6 sm:px-10 py-8 sm:py-10 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-5">
          ...
          {NEXT_DROP_LABEL && <span className="text-[11px] uppercase tracking-[0.18em] text-foreground/50">Next drop {NEXT_DROP_LABEL}</span>}
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="relative rounded-[16px] border border-foreground bg-bg-dark text-primary-foreground px-6 sm:px-10 py-8 sm:py-12 mb-8 overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <span className="inline-block px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] rounded-[6px] bg-primary-foreground text-bg-dark">{banner.badge}</span>
      ...
      {pct > 0 && <p className="text-lg sm:text-2xl font-semibold tracking-tight mt-2">Hasta {pct}% extra sobre tu precio mayorista</p>}
      ...
      <div className="flex flex-wrap items-center gap-3 mt-6">
        ...
        {banner.secondary && <span className="text-[11px] uppercase tracking-[0.18em] text-primary-foreground/50 sm:ml-2">{banner.secondary}</span>}
      </div>
