# Solución: Mobile Zoom para ProductoClient.tsx

A continuación tenés el código completo del componente `ProductoClient.tsx` con la funcionalidad del zoom para móviles incluida.

Una vez que tengas este código o lo hayas aplicado en la rama nueva, avisame y ejecuto los comandos para **eliminar nuestra rama actual y volver a `main`**, dejándote el repositorio completamente limpio para que vos y tu compañero se organicen.
...
export default function ProductoClient({ slug }: { slug: string }) {
  ...
  const handleGalleryDrag = (e: React.MouseEvent | React.TouchEvent) => {
    if (galleryZoom === 1) return;
    // Simple drag logic could be added here if needed, but for now pinch-zoom is supported by browser
  };
    ...
    <>
      ...
      {isGalleryOpen && (
        <div className="fixed inset-0 z-[300] bg-black flex flex-col animate-in fade-in duration-200 overflow-hidden">
          ...
          <div className="flex-1 relative overflow-hidden flex items-center justify-center cursor-zoom-in"
            onTouchStart={galleryZoom === 1 ? handleTouchStart : undefined}
            onTouchEnd={galleryZoom === 1 ? handleTouchEnd : undefined}
            onClick={toggleGalleryZoom}>
            <div className="relative w-full h-full transition-transform duration-300 ease-out"
                 style={{ 
                   transform: `scale(${galleryZoom}) translate(${galleryOffset.x}px, ${galleryOffset.y}px)`,
                   touchAction: galleryZoom > 1 ? 'none' : 'pan-y' 
                 }}>
              <Image 
