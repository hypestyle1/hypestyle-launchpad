...
// Fetches all products without WP-side category filter; filter client-side
// so CATEGORY_MAP names don't need to match WP slugs exactly.
export function useProducts(first = 20, category?: string) {
  return useQuery<NormalizedProduct[]>({
    ...
    queryFn: async (): Promise<NormalizedProduct[]> => {
      try {
        const data = await fetchGraphQL<{ products: { nodes: any[] } }>(
          GET_PRODUCTS,
          { first },
        );
