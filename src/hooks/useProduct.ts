...
function parsePrice(s?: string | null): number {
  if (!s) return 0;
  // ARS: "$ 135.000,00" → strip non-digits/comma → "135000,00" → 135000
...
function fromWPNode(node: any): Product {
  ...
  // Images: featured first, then gallery
  ...
  if (variations.length) {
    ...
    const sz = 'Única';
    sizes.push(sz);
    stock[sz] = stockStatus(node.stockStatus || 'IN_STOCK', node.stockQuantity ?? null);
  }
  ...
  const careItems = isAccessory ? CARE_ACCESSORY : CARE_APPAREL;
...
export function useProduct(slug: string | undefined) {
  ...
  return useQuery<Product | undefined>({
    ...
    enabled: !!slug,
    staleTime: 2 * 60 * 1000, // 2 min — product pages don't need real-time freshness
