...
const mobilePanels = {
  ...
  arriba: {
    ...
    verTodo: "/arriba/",
    items: [
      { label: "Hoodies",    href: "/hoodies/", panel: null },
      { label: "Remeras",    href: "/tees/",    panel: null },
      { label: "Accesorios", href: "/accesorios/", panel: null },
    ],
  },
  abajo: {
    title: "Abajo",
    verTodo: "/abajo/",
    items: [
      { label: "Pantalones", href: "/abajo/",  panel: null },
      { label: "Jorts",      href: "/abajo/",  panel: null },
    ],
  },
  accesorios: {
    title: "Accesorios",
    verTodo: "/accesorios/",
    items: [
      { label: "Gorras",   href: "/accesorios/", panel: null },
      { label: "Beanies",  href: "/accesorios/", panel: null },
      { label: "Rings",    href: "/accesorios/", panel: null },
    ],
  },
};
...
export default function Navbar() {
  ...
  // Reset mobile panel when closing menu
  useEffect(() => {
  ...
  // Smart nav: if on home scroll to section, if on other page go to dedicated page
    ...
    <>
      ...
      >
          ...
          {searchOpen ? (
            ...
            <>
              ...
              <div className="hidden lg:flex items-center gap-1 flex-1">
                {navLinks.map((link) => {
                  const isActive = link.routeMatch && location.pathname.startsWith(link.routeMatch);
                  return (
              ...
              {/* Logo — desktop: centered absolute / mobile: next to hamburger */}
              <Link to="/" className="hidden lg:flex absolute left-1/2 -translate-x-1/2 items-center z-10">
                {!logoError ? (
              ...
              <div className="lg:hidden flex items-center gap-2.5 flex-none">
                ...
                <Link to="/">
                  {!logoError ? (
                    <img src="/logo-hypestyle-2026.png" alt="Hypestyle"
                      className="w-auto"
                      style={{ height: scrolled ? "18px" : "22px" }}
                      onError={() => setLogoError(true)} />
                  ) : (
                    <span className="text-[15px] font-normal tracking-[0.08em] text-foreground">Hypestyle</span>
                  )}
              ...
              <div className="flex items-center justify-end gap-3 flex-1">
                ...
                <button
                  ...
                  className="relative p-1.5 rounded-[8px] hover:bg-black/[0.06] transition-colors duration-150"
            ...
            <div className="max-w-[1400px] mx-auto px-4 py-8 grid grid-cols-[1fr_1fr_1fr_280px] gap-8">
              ...
              <div>
                ...
                {megaMenu.general.map((l) => (
                  <button key={l.label} onClick={() => handleSmartLink(l.href, l.homeHash)}
                    ...
                    {l.label}
                    {l.isNew && (
                      <span className="text-[9px] font-bold uppercase tracking-wider bg-foreground text-background px-1.5 py-0.5 rounded-[4px]">NEW</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div>
                ...
                {megaMenu.categorias.map((l) => (
                  <Link key={l.label} to={l.href} onClick={() => setShopOpen(false)}
                    className={`block text-[13px] text-foreground/70 hover:text-foreground transition-colors mb-2.5 ${l.indent ? "pl-3 text-foreground/50 before:content-['↳'] before:mr-1.5 before:text-foreground/25" : "font-medium"}`}>
                    {l.label}
                  </Link>
                ))}
              </div>
              ...
              <div className="relative overflow-hidden rounded-sm">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end p-4">
                  <div>
                    ...
                    <Link to="/camo-set-drop/" onClick={() => setShopOpen(false)}
        ...
        {searchOpen && searchQuery.length >= 2 && (
          <div
            className="absolute left-0 right-0 top-full animate-in fade-in slide-in-from-top-1 duration-200"
            style={{ ...glassStyle, borderRadius: "0 0 14px 14px", borderTop: "none" }}
          >
        ...
        <div className="fixed inset-0 z-50 flex flex-col overflow-hidden"
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-foreground/8"
            ...
            {mobilePanel !== "main" ? (
              ...
              <button onClick={closeMobile} aria-label="Cerrar">
                <X className="w-5 h-5" strokeWidth={1.2} />
              </button>
            )}

            <Link to="/" onClick={closeMobile} className="absolute left-1/2 -translate-x-1/2">
              {!logoError ? (
          ...
          {mobilePanel !== "main" && (
            <div className="px-6 pt-6 pb-2">
              ...
              {"verTodo" in panel && (
                <Link to={panel.verTodo} onClick={closeMobile}
              ...
              <>
                ...
                <div className="mb-6">
                  ...
                  {mobilePanels.main.collections.map((l) => (
                    <Link key={l.label} to={l.href} onClick={closeMobile}
