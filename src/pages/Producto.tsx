  ...
  return (
    <div className="border-t border-border">
      ...
      <div className="overflow-hidden transition-all duration-300" style={{ maxHeight: open ? "400px" : "0px" }}>
        <div className="pb-4 text-[13px] text-foreground/65 leading-relaxed whitespace-pre-line">
          {children}
        </div>
      </div>
    </div>
      ...
      <div className="bg-white w-full max-w-[520px] shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 bg-[#f8f8f6] border-b border-border space-y-1">
          <p className="text-[12px] text-foreground/70">
            <span className="font-semibold text-foreground">Modelo (hombre)</span> — 1.80m de altura, talle <span className="font-semibold">M</span>
          </p>
          <p className="text-[12px] text-foreground/70">
            <span className="font-semibold text-foreground">Modelo (mujer)</span> — 1.68m de altura, talle <span className="font-semibold">S</span>
          </p>
        </div>
        <div className="px-6 py-5">
          ...
          <p className="text-[11px] text-muted-foreground mt-4 leading-relaxed">
            Las medidas son del cuerpo, no de la prenda. Si estás entre dos talles, te recomendamos el más grande.
          </p>
        </div>
      </div>
    </div>
...
export default function Producto() {
    ...
    return (
      <>
        ...
        <main className="pt-[var(--offset)] flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <button onClick={() => navigate("/")} className="text-[12px] underline hover:text-foreground transition-colors">
            Volver al inicio
          </button>
        </main>
        <Footer />
  ...
  // Sticky bar: aparece cuando el botón principal sale del viewport
  useEffect(() => {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
            ...
            <div className="flex gap-3">
              ...
              <div className="hidden md:flex flex-col gap-2 w-[72px] flex-shrink-0">
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`w-full aspect-square overflow-hidden border-[1.5px] transition-colors ${
              ...
              <div className="flex-1 flex flex-col gap-3">
                <div
                  className="relative aspect-square overflow-hidden bg-bg-alt select-none cursor-crosshair"
                ...
                >
                  <img
                    ...
                    style={{
                      ...
                      transition: zoomPos ? "transform 0.1s ease" : "transform 0.3s ease",
                  ...
                  {selectedImage > 0 && (
                    <button onClick={() => setSelectedImage((p) => p - 1)}
                      className="md:hidden absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/80 backdrop-blur-sm flex items-center justify-center shadow-sm"
                      aria-label="Anterior">
                  ...
                  {selectedImage < product.images.length - 1 && (
                    <button onClick={() => setSelectedImage((p) => p + 1)}
                      className="md:hidden absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/80 backdrop-blur-sm flex items-center justify-center shadow-sm"
                  ...
                  {product.images.map((_, i) => (
                    <button key={i} onClick={() => setSelectedImage(i)}
                      className={`transition-all duration-200 rounded-full ${
                        i === selectedImage ? "w-4 h-1.5 bg-foreground" : "w-1.5 h-1.5 bg-foreground/25"
                      }`}
            ...
            <div className="flex flex-col">
              ...
              {/* Color selector — solo si hay más de una variante */}
                  ...
                  <div className="flex gap-2">
                    {product.colors.map((c) => (
                      <button
                        ...
                        title={c.label}
                        aria-label={c.label}
                        className={`w-[44px] h-[44px] overflow-hidden border transition-colors duration-150 rounded-[5px] ${
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-[0.08em] border border-border px-2.5 py-1 rounded-[10px]">
                  {product.fit}
                </span>
              </div>
              ...
              <div className="border-b border-border">
                ...
                <Accordion title="Envíos y devoluciones">
                  ...
                  <a href="/politicas-de-devolucion/" className="underline text-foreground hover:text-foreground/70 transition-colors mt-2 block">
      ...
      {/* ── Sticky bar mobile ── */}
      <div
        className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-border px-4 py-3 flex items-center gap-3 transition-transform duration-300 ${
      ...
      {/* ── Popup guía de talles ── */}
      {sizeGuideOpen && <SizeGuideModal onClose={() => setSizeGuideOpen(false)} />}

      {/* ── Mini popup añadido al carrito ── */}
      {added && (
        <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/30"
          ...
          <div className="bg-white w-full max-w-[480px] shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-3 p-5 border-b border-border">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Talle: {selectedSize} · {formatPrice(product.price)}
                </p>
              </div>
            ...
            <div className="p-5 border-b border-border">
              <p className="text-[11px] uppercase tracking-[0.12em] text-muted-foreground mb-3">Mejora tu compra</p>
                ...
                {related.map((p) => (
                  <div key={p.name}>
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      <p className="text-[10px] font-semibold text-muted-foreground">{formatPrice(p.price)}</p>
                      <button
                        onClick={() => add({ id: p.name, name: p.name, price: p.price, image: p.image, size: "U", quantity: 1 })}
                        className="w-5 h-5 rounded-full border border-foreground/30 text-foreground flex items-center justify-center text-[13px] font-light leading-none hover:bg-foreground hover:text-white transition-colors"
                      >
                        +
                      </button>
                    </div>
            ...
            <div className="p-5 space-y-2">
              <button
                onClick={() => { setAdded(false); setDrawerOpen(true); }}
