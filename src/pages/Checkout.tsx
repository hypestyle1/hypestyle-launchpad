...
export default function Checkout() {
  ...
  const [info, setInfo] = useState<InfoForm>({
    ...
    nombre: "", apellido: "", dni: "",
    direccion: "", depto: "", cp: "",
    ciudad: "", provincia: "Buenos Aires", telefono: "",
  });
            ...
            <form onSubmit={handleInfoSubmit} className="space-y-6">
              ...
              <div>
                ...
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  ...
                  />
                  <span className="text-[12px] text-muted-foreground">
                    Recibir novedades, drops y acceso anticipado
                  </span>
                </label>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input placeholder="Nombre" required value={info.nombre}
                    ...
                    <input placeholder="Apellido" required value={info.apellido}
                  ...
                  <input placeholder="Dirección y número" required value={info.direccion}
                    ...
                    className="w-full border border-border px-4 py-3 text-[13px] focus:outline-none focus:border-foreground transition-colors rounded-[10px]" />
                  <input placeholder="Departamento / Piso (opcional)" value={info.depto}
                    onChange={(e) => setInfo({ ...info, depto: e.target.value })}
                    className="w-full border border-border px-4 py-3 text-[13px] focus:outline-none focus:border-foreground transition-colors rounded-[10px]" />
                  <div className="grid grid-cols-2 gap-2">
                    <input placeholder="Código postal" required value={info.cp}
                      onChange={(e) => setInfo({ ...info, cp: e.target.value })}
                      className="border border-border px-4 py-3 text-[13px] focus:outline-none focus:border-foreground transition-colors rounded-[10px]" />
                    <input placeholder="Ciudad" required value={info.ciudad}
                      onChange={(e) => setInfo({ ...info, ciudad: e.target.value })}
                      className="border border-border px-4 py-3 text-[13px] focus:outline-none focus:border-foreground transition-colors rounded-[10px]" />
                  </div>
                  <input placeholder="Teléfono (con código de área)" required value={info.telefono}
              ...
              <button type="submit"
                className="w-full bg-bg-dark text-primary-foreground py-4 text-[12px] font-bold uppercase tracking-[0.1em] hover:bg-bg-dark/85 transition-colors rounded-[10px]">
              ...
              <div>
                ...
                <label className="flex items-center justify-between border border-foreground px-4 py-4 cursor-pointer bg-blue-50/30 rounded-[10px]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[13px] font-medium">Andreani — Envío a domicilio</p>
                      <p className="text-[11px] text-muted-foreground">5 a 10 días hábiles</p>
                    </div>
                  </div>
                  {freeShipping ? (
                    <div className="text-right">
                      <span className="text-[12px] text-muted-foreground line-through block">
                        {formatPrice(ENVIO_COSTO)}
                      </span>
            ...
            <form onSubmit={handlePagoSubmit} className="space-y-6">
                ...
                <div className="flex items-center justify-between px-4 py-3">
                  <div className="flex gap-2">
                    ...
                    <span>
                      Andreani ·{" "}
                      {freeShipping ? (
                        <><span className="line-through text-muted-foreground">{formatPrice(ENVIO_COSTO)}</span>{" "}<span className="text-green-700 font-semibold">Gratis</span></>
                      ) : (
                        formatPrice(ENVIO_COSTO)
                      )}
                    </span>
              ...
              <div>
                <p className="text-[13px] font-semibold mb-1">
                  Dejanos tu Instagram para sumarte a Close Friends y acceder antes a drops, preventas y beneficios exclusivos.
                </p>
                <p className="text-[11px] text-muted-foreground mb-2">Campo opcional</p>
                <textarea
                  value={pago.instagram}
                  onChange={(e) => setPago({ ...pago, instagram: e.target.value })}
                  placeholder="Escribí acá tu usuario de Instagram"
                  rows={2}
              ...
              <div>
                ...
                {pago.metodo === "transferencia" ? (
                  <div className="border border-foreground rounded-[10px] overflow-hidden">
                    <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border">
                      <button
                        type="button"
                        onClick={() => setPago({ ...pago, metodo: "" })}
                        className="text-[12px] text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1"
                      >
                        ‹
                      </button>
                    ...
                    <div className="px-4 py-4 bg-foreground/[0.02]">
                      ...
                      <button
                        type="button"
                        onClick={() => setPago({ ...pago, metodo: "" })}
                        className="text-[12px] underline text-muted-foreground hover:text-foreground transition-colors mt-2 block"
                      >
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <label
                        key={m.id}
                        className={`flex items-center gap-3 border px-4 py-3.5 cursor-pointer transition-colors rounded-[10px] ${
                          pago.metodo === m.id ? "border-foreground bg-foreground/[0.03]" : "border-border hover:border-foreground/40"
                        }`}
                      >
                        <input
                          type="radio"
                          name="metodo"
                          value={m.id}
                          checked={pago.metodo === m.id}
                          onChange={() => setPago({ ...pago, metodo: m.id })}
                ...
                {!pago.metodo && (
                  <p className="text-[11px] text-destructive mt-1">Seleccioná un medio de pago</p>
                )}
              </div>
          ...
          <div className="sticky top-6">
            ...
            <div className="space-y-2 border-t border-border pt-4">
              ...
              <div className="flex justify-between text-[13px]">
                ...
                <span>
                  {step === "info"
                    ...
                    : freeShipping
                      ? <><span className="line-through text-muted-foreground mr-1">{formatPrice(ENVIO_COSTO)}</span><span className="text-green-700 font-semibold">Gratis</span></>
                      : formatPrice(ENVIO_COSTO)
                  }
                </span>
              </div>
              <div className="flex justify-between text-[16px] font-bold border-t border-border pt-3 mt-2">
                <span>Total</span>
                <span>{formatPrice(step === "info" ? subtotal - descuento : totalFinal)}</span>
              </div>
              {step === "pago" && (
                <p className="text-[11px] text-muted-foreground">
                  {currency === "ARS" && <>Con transferencia bancaria: <span className="font-semibold text-green-700">{formatPrice(transferTotal)}</span></>}
                </p>
              )}
            </div>
          </div>
